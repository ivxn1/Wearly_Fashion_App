...
class BrandBaseForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if name and not name.replace(" ", "").isalpha():
            self.add_error(
                "name", "Brand name must contain only alphabetic characters and spaces."
            )
...
class GarmentBaseForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["category"].choices = [
            (value, label)
            for value, label in GARMENT_CATEGORY_CHOICES
            if value != "All"
        ]
    ...
    def clean(self):
        ...
        if title and not title.replace(" ", "").isalnum():
            self.add_error(
                "title", "Title must contain only alphanumeric characters and spaces."
            )

        if color and not color.replace(" ", "").isalpha():
            self.add_error(
                "color", "Color must contain only alphabetic characters and spaces."
            )

        if size and not size.replace(" ", "").isalnum():
            self.add_error(
                "size", "Size must contain only alphanumeric characters and spaces."
            )

        if material and not material.replace(" ", "").isalpha():
            self.add_error(
                "material",
                "Material must contain only alphabetic characters and spaces.",
            )
...
class GarmentSearchForm(forms.Form):
    ...
    season = forms.ChoiceField(
        required=False, label="Season", choices=SeasonChoices, widget=forms.Select
    )
