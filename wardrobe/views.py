...
class GarmentDeleteView(IsUserOwnerMixin, LoginRequiredMixin, DeleteView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if self.object.outfitgarment_set.exists():
            messages.error(
                request,
                f'Cannot delete "{self.object.title}" because it '
                f"is part of one or more outfits. "
                f"Please remove it from all outfits before deleting.",
            )
...
class BrandListView(LoginRequiredMixin, SetPaginateByMixin, ListView, FormView):
    ...
    def get_queryset(self):
        qs = (
            Brand.objects.annotate(garment_count=Count("wardrobe"))
            .order_by("name")
            .all()
        )
        form = self.get_form()
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context["most_used_in_outfits"] = (
            Brand.objects.annotate(
                outfit_count=Count("wardrobe__outfitgarment__outfit", distinct=True)
            )
            .order_by("-outfit_count")
            .first()
        )
        context["most_expensive_brand"] = (
            Brand.objects.annotate(avg_price=Avg("wardrobe__price"))
            .filter(avg_price__isnull=False)
            .order_by("-avg_price")
            .first()
        )
        context["most_garments_brand"] = (
            self.get_queryset().order_by("-garment_count").first()
        )
        context["form"] = self.get_form()
        context["paginate_by"] = self.get_paginate_by(self.get_queryset())
        return context
...
class BrandDetailsView(LoginRequiredMixin, DetailView):
    ...
    def get_queryset(self):
        return Brand.objects.annotate(garment_count=Count("wardrobe")).all()
...
class BrandDeleteView(LoginRequiredMixin, DeleteView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if self.object.garment_count > 0:
            messages.error(
                request,
                f'Cannot delete "{self.object.name}" because'
                f"it has {self.object.garment_count} garment(s) connected to it. "
        ...
        messages.success(
            request, f'Brand "{self.object.name}" has been successfully deleted.'
        )
        return super().post(request, *args, **kwargs)
