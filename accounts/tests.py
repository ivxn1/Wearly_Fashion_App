...
class CustomerUserModelTest(TestCase):
    ...
    def test_user_str_returns_email(self):
        user = UserModel.objects.create_user(email="str@wearly.app", password="pass123")
...
class ProfileModelTest(TestCase):
    ...
    def test_profile_created_by_signal(self):
        ...
        self.assertTrue(CustomerProfileModel.objects.filter(user=user).exists())
...
class ActivatePremiumViewTest(TestCase):
    ...
    def test_valid_token_activates_premium(self):
        ...
        response = self.client.get(reverse("activate-premium", kwargs={"token": token}))
    ...
    def test_already_premium_user_stays_premium(self):
        ...
        self.user.save()
        token = signing.dumps({"user_id": self.user.pk}, salt=PREMIUM_SALT)
        self.client.get(reverse("activate-premium", kwargs={"token": token}))
        self.user.refresh_from_db()
        self.assertTrue(self.user.is_premium)
